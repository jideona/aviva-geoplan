...
export default function ProjectDataScreen() {
  ...
  const [query,setQuery]=useState(''); const [filter,setFilter]=useState<Filter>('all'); const [email,setEmail]=useState(''); const [syncing,setSyncing]=useState(false);
  ...
  const visible=useMemo(()=>features.filter(f=>{const p=f.properties||{}; if(filter==='mine'&&(!email||!([p.surveyed_by,p.last_edited_by,p.name_recorded_by].filter(Boolean).includes(email))))return false; if(filter==='recent'){const d=new Date(p.updated_at||p.created_at||0).getTime(); if(Date.now()-d>30*86400000)return false;} if(filter==='attention'&&!needsAttention(f))return false; const q=query.trim().toLowerCase(); if(q&&!`${title(f)} ${subtitle(f)} ${p.surveyed_by||''} ${p.last_edited_by||''}`.toLowerCase().includes(q))return false; return true;}),[features,filter,email,query]);
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.body}>
      <View style={s.summary}>{Object.entries(KIND_LABEL).map(([k,m])=><View key={k} style={s.summaryRow}><Icon name={m.icon} size={18} color={COLOR.primary700}/><Text style={s.summaryLabel}>{m.label}</Text><Text style={s.summaryCount}>{counts[k]??0}</Text></View>)}</View>
