...
export default function ProjectDataScreen({
  ...
  function openRecord(f:any) {
    const p = f.properties || {};
    const serverId = p.id || f.id;
    if (!serverId) {
      notify('Record detail', 'This record does not have a server id yet.');
      return;
    }
    ...
    const kindMap: Record<string,string> = {
      ...
      streets: 'street',
      routes: 'route',
      building_photos: 'building_photo',
    };

    const kind = kindMap[f._cache_kind];
    if (!kind) return;
    ...
    setDetailItem({
      clientId: `server:${serverId}`,
      kind,
      label: title(f),
      sub: subtitle(f),
      lat,
      lon,
      accuracy: p.gps_accuracy_m != null ? Number(p.gps_accuracy_m) : undefined,
      createdAt: p.created_at || p.updated_at || new Date().toISOString(),
      synced: true,
      serverId: String(serverId),
      source: 'server',
      needsAttention: needsAttention(f),
    });
  }
  ...
  return <View style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.body}>
      ...
      {visible.map((f,i)=>{
        const meta=KIND_LABEL[f._cache_kind]??{label:f._cache_kind,icon:'data' as IconName};
        return (
          <TouchableOpacity
            key={`${f._cache_kind}:${prop(f,'id')}:${i}`}
            style={[s.card, needsAttention(f) && s.cardAttention]}
            activeOpacity={0.7}
            onPress={() => openRecord(f)}
          >
            <View style={s.iconWell}>
              <Icon name={meta.icon} size={19} color={COLOR.primary900}/>
            </View>

            <View style={{flex:1,minWidth:0}}>
              <Text style={s.cardTitle} numberOfLines={1}>{title(f)}</Text>
              <Text style={s.cardSub} numberOfLines={2}>{subtitle(f)||meta.label}</Text>
              <Text style={s.cardMeta}>
                {prop(f,'last_edited_by')
                  ? `Edited by ${prop(f,'last_edited_by')}`
                  : prop(f,'surveyed_by')
                    ? `Captured by ${prop(f,'surveyed_by')}`
                    : 'Baseline project data'}
              </Text>
            </View>

            {needsAttention(f)
              ? <Icon name="flagged" size={18} color={COLOR.accent500}/>
              : <Icon name="chevronRight" size={15} color={COLOR.text500}/>}
          </TouchableOpacity>
        );
      })}
      ...
      {!visible.length&&(
        <Text style={s.empty}>
          No records match this view. Sync project data if this device has not been refreshed yet.
        </Text>
      )}
    </ScrollView>
