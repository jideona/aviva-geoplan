...
export default function RouteCaptureScreen({
  ...
  useEffect(() => { (async()=>{ const pid=await kvGet('projectId'); setProjectId(pid); if(pid){
    if (edit) {
      setRouteType(edit.route_type ?? edit.type ?? 'cable_route');
      setCode(edit.code ?? '');
      setNotes(edit.notes ?? '');
      return;
    }
  ...
  async function saveEdit() {
    ...
    notify('Route updated', 'Changes queued for sync.');
    onSaved();
  }
  ...
  async function discard() {
    ...
    if (projectId) await saveActiveRoute(projectId, null);
    onCancel();
  }
  ...
  return <ScrollView contentContainerStyle={s.body}>
    ...
    <View style={s.stats}>
      <Stat
        label="DISTANCE"
        value={`${Math.round(edit ? Number(edit.length_m ?? 0) : lengthM(points))} m`}
      />
      <Stat
        label="POINTS"
        value={String(edit ? Number(edit.point_count ?? edit.points ?? 0) : points.length)}
      />
      <Stat
        label="ELAPSED"
        value={edit
    ...
    <Text style={s.label}>Route code</Text>
    <TextInput
      style={[s.input, { minHeight: MIN_TOUCH }]}
      value={code}
      onChangeText={setCode}
      placeholder="Route code (optional)"
      placeholderTextColor={COLOR.text500}
    />
    ...
    <TextInput
      ...
      placeholder="Existing duct, obstruction, proposed trench…"
      placeholderTextColor={COLOR.text500}
      multiline
    />
    {edit ? (
      <>
        <Text style={s.help}>
          Route type, code and notes can be updated here. The recorded GPS track is preserved.
        </Text>
        <TouchableOpacity style={s.primary} onPress={saveEdit}>
          <Text style={s.primaryText}>Save route update</Text>
        </TouchableOpacity>
      </>
    ) : (
      <>
        {!recording
          ? <TouchableOpacity style={s.primary} onPress={start}>
              <Text style={s.primaryText}>{points.length ? 'Resume recording' : 'Start recording'}</Text>
            </TouchableOpacity>
          : <TouchableOpacity style={s.pause} onPress={pause}>
              <Text style={s.primaryText}>Pause recording</Text>
            </TouchableOpacity>}
        {points.length >= 2 && (
          <TouchableOpacity style={s.finish} onPress={finish}>
            <Text style={s.finishText}>Finish & save route</Text>
          </TouchableOpacity>
        )}
      </>
    )}
